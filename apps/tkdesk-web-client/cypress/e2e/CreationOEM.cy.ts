...
describe('Create OEM User', () => {
  ...
  const user =
    Cypress.env('VITE_ENV') === 'development' ? 'user1499' : 'user1079' // user1079: staging, user1499: dev
  beforeEach(() => {
  ...
  it('air_cooled_condenser mode:verify', () => {
    cy.wait('@resetStatus').then(() => {
      ...
      cy.inputSelect('cy.input_data.fluid_c1', 'cy.r134a', '.ant-collapse')
